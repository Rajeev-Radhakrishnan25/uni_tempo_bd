...
class RideRequestTest {
    ...
    void setUp() {
        ...
        rideRequestService = new RideRequestService(rideRequestRepository, rideRepository, userRepository, emailUtil, mapper);
        bannerId = "B12345678";
    ...
    void createRideRequest_Success() {
        ...
        when(mapper.map(rideRequest, RideRequestResponse.class)).thenReturn(rideRequestResponse);
    ...
    void getPendingRideRequestsByDriver_Success() {
        ...
        when(mapper.map(pendingRequest1, RideRequestResponse.class)).thenReturn(response1);
        when(mapper.map(pendingRequest2, RideRequestResponse.class)).thenReturn(response2);
    ...
    void updateRideRequestStatus_Success() {
        ...
        when(mapper.map(any(RideRequest.class), eq(RideRequestResponse.class))).thenReturn(
    ...
    void updateRideRequestStatus_SendsAcceptedNotification(String statusInput, String expectedStatus, boolean isAccepted) {
        ...
        when(rideRequestRepository.save(any(RideRequest.class))).thenReturn(pendingRequest);
        when(mapper.map(any(RideRequest.class), eq(RideRequestResponse.class))).thenReturn(
                RideRequestResponse.builder().rideRequestId(request.getRideRequestId()).status(expectedStatus).build()
        );
    ...
    void updateRideRequestStatus_RejectRequest_AddsBackAvailableSeats() {
        ...
        when(mapper.map(any(RideRequest.class), eq(RideRequestResponse.class))).thenReturn(
                RideRequestResponse.builder().rideRequestId(request.getRideRequestId()).status("REJECTED").build()
        );
    ...
    void getRideRequestsByRider_Success() {
        ...
        when(mapper.map(pendingRequest, RideRequestResponse.class)).thenReturn(response1);
        when(mapper.map(acceptedRequest, RideRequestResponse.class)).thenReturn(response2);
        when(mapper.map(rejectedRequest, RideRequestResponse.class)).thenReturn(response3);
